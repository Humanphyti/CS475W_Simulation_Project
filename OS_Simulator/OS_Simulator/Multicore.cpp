...
// Implements a Multi-core scheduler using 4 cores running FCFS
...
void Multicore(vector<PCB> &pcbs) {
	queue<PCB> ready, FCFS1, FCFS2, FCFS3, FCFS4;
	...
	do {
		...
		for (int i = 0; i < io_vector.size(); i++) {
			...
			if (ready.empty() && FCFS1.empty() && FCFS2.empty() && FCFS3.empty() && FCFS4.empty()) {
			...
			//else the ready queue is not empty, so get the next process
				...
				if (FCFS1.empty()) {
					*current_PCB = FCFS1.front();
					ready.pop();

				}
				else if (FCFS2.empty()) {
					*current_PCB = FCFS2.front();
					ready.pop();
				}
				else if (FCFS3.empty()) {
					*current_PCB = FCFS3.front();
					ready.pop();
				}
				else if (FCFS4.empty()) {
					*current_PCB = FCFS4.front();
					ready.pop();
				}
